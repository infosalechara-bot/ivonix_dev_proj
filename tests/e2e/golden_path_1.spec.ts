...
// Base URL must be the ontology origin only (e.g. http://host:8081), not .../api/v1.
if (/\/api\/v1\/?$/i.test(API)) {
  throw new Error(`PULSE_E2E_API_URL must not end with /api/v1 (got ${API}). Use the server origin only; the harness appends /api/v1/... paths.`);
}

async function api(path:string, init:RequestInit={}) {
  const url=`${API}${path}`;
  const r=await fetch(url,{...init,headers:{'content-type':'application/json','authorization':`Bearer ${USER_TOKEN}`,...(init.headers??{})}});
...
if (activation.r.status !== 200) {
  throw new Error(
    ...
    `url: ${activation.url}\n` +
    `body: ${typeof activation.body === 'string' ? activation.body.slice(0,500) : JSON.stringify(activation.body)}\n` +
    `hint: PULSE_E2E_API_URL must point at the Java ontology API (port 8081), not Vercel/frontend/Supabase. Route is POST /api/v1/key/activation-codes on UniversalKeyController.`
  );
}
