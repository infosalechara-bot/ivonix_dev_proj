...
final class CanonicalPulseEvent {
    ...
    static Map<String, Object> build(
            ...
            UUID legacyEventId,
            Instant occurredAt) {
        if (eventType == null || !eventType.matches("^[a-z][a-z0-9]*(\\.[a-z0-9]+)+$")) {
            throw new IllegalArgumentException("Event type is not canonical: " + eventType);
        }
        ...
        String aggregateId = payload != null && payload.get("deviceId") != null
                ? String.valueOf(payload.get("deviceId")) : legacyEventId.toString();
    ...
    private static String write(ObjectMapper mapper, Object value) {
        try {
            ...
            throw new IllegalArgumentException("Unable to serialize canonical event", e);
        }
    }
    ...
    private static String hmacHex(byte[] key, String value) {
        try {
            ...
            throw new IllegalStateException("Canonical event signing unavailable", e);
        }
    }

    private static String ulid(long millis, UUID source) {
        byte[] random = ByteBuffer.allocate(16)
                .putLong(source.getMostSignificantBits())
                .putLong(source.getLeastSignificantBits())
                .array();
        StringBuilder out = new StringBuilder(26);
        long time = millis;
        for (int shift = 40; shift >= 0; shift -= 5) out.append(CROCKFORD[(int) ((time >>> shift) & 31)]);
        int buffer = 0, bits = 0;
        for (byte b : random) {
            ...
            while (bits >= 5 && out.length() < 26) {
                bits -= 5;
                out.append(CROCKFORD[(buffer >>> bits) & 31]);
            }
        }
