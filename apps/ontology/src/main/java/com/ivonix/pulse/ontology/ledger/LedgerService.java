...
public class LedgerService {
    ...
    public LedgerService(JdbcTemplate jdbc, ObjectMapper mapper) { this.jdbc = jdbc; this.mapper = mapper; }
    ...
    public UUID submitTransaction(UUID organizationId, String txType, Map<String,Object> payload, UUID userId) {
        ...
        jdbc.queryForObject("SELECT pg_advisory_xact_lock(hashtextextended(?,0))", Long.class, organizationId.toString());
        ...
        String txHash = HashUtil.sha256(organizationId + "|" + txId + "|" + txType + "|" + canonicalPayload + "|" + now);
        String blockHash = HashUtil.sha256(organizationId + "|" + number + "|" + previousHash + "|" + txHash);
        UUID blockId = UUID.randomUUID();
        jdbc.update("INSERT INTO ledger_blocks(id,organization_id,block_number,previous_block_hash,block_hash,block_timestamp,transaction_count,created_by) VALUES(?,?,?,?,?,?,?,?)", blockId, organizationId, number, previousHash, blockHash, Timestamp.from(now), 1, userId);
        jdbc.update("INSERT INTO ledger_transactions(id,organization_id,block_id,tx_type,payload,tx_hash,created_at,created_by) VALUES(?,?,?,?,?::jsonb,?,?,?)", txId, organizationId, blockId, txType, canonicalPayload, txHash, Timestamp.from(now), userId);
        return txId;
    }
    ...
    public Map<String,Object> verifyChain(UUID organizationId, UUID userId) {
        ...
        String previous = GENESIS; long expected = 0;
        for (Map<String,Object> b : blocks) {
            ...
            List<Map<String,Object>> txs = jdbc.queryForList("SELECT * FROM ledger_transactions WHERE organization_id=? AND block_id=? ORDER BY id ASC", organizationId, b.get("id"));
            ...
            String payload = canonicalJson(Objects.toString(tx.get("payload")));
            Object createdValue = tx.get("created_at");
            Instant created = createdValue instanceof Timestamp t ? t.toInstant() : Instant.parse(createdValue.toString());
            String txHash = HashUtil.sha256(organizationId + "|" + tx.get("id") + "|" + tx.get("tx_type") + "|" + payload + "|" + created);
    ...
    private Map<String,Object> latestBlock(UUID org) { List<Map<String,Object>> rows = jdbc.queryForList("SELECT * FROM ledger_blocks WHERE organization_id=? ORDER BY block_number DESC LIMIT 1", org); return rows.isEmpty()?null:rows.get(0); }
    private void requireMember(UUID org, UUID user) { Integer count=jdbc.queryForObject("SELECT count(*) FROM organization_members WHERE organization_id=? AND user_id=?",Integer.class,org,user); if(count==null||count==0) throw new SecurityException("Organization membership required"); }
    private String canonical(Object value) { try{return mapper.writeValueAsString(mapper.readTree(mapper.writeValueAsString(value)));}catch(JsonProcessingException e){throw new IllegalArgumentException("Payload must be JSON serializable",e);} }
    private String canonicalJson(String json) { try{return mapper.writeValueAsString(mapper.readTree(json));}catch(JsonProcessingException e){throw new IllegalStateException("Stored payload is invalid JSON",e);} }
