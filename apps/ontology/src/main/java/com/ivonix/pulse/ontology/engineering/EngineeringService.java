...
public class EngineeringService {
    ...
    @Transactional
    public Map<String, Object> diagnose(UUID deviceId, UUID orgId, UUID userId,
                                        List<UUID> evidenceIds, Map<String, Object> telemetry) {
        requireDeviceAccess(deviceId, orgId, userId);

        List<String> types = (evidenceIds == null || evidenceIds.isEmpty())
                ? List.of()
                : jdbc.queryForList(
                    "select evidence_type from public.inspection_evidence " +
                    "where device_id = ? and organization_id = ? and id = any(?::uuid[])",
                    String.class, deviceId, orgId, evidenceIds.toArray(new UUID[0]));

        String domain = jdbc.queryForObject(
                "select domain from public.machine_dna where device_id = ?",
                String.class, deviceId);

        Map<String, Object> req = Map.of(
                "telemetry", telemetry == null ? Map.of() : telemetry,
                "evidenceTypes", types,
                "deviceDomain", domain == null ? "unknown" : domain);
        ...
        jdbc.update(
                ...
                "id, device_id, organization_id, probable_fault, confidence, " +
                "evidence, recommended_actions, do_not_disassemble, created_by) " +
                "values(?,?,?,?,?,?::jsonb,?::jsonb,?,?)",
                id, deviceId, orgId,
                String.valueOf(response.getOrDefault("probableFault", "Unknown")),
                ((Number) response.getOrDefault("confidence", 0)).doubleValue(),
                toJson(Map.of("types", types)),
                toJson(response.getOrDefault("recommendedActions", List.of())),
                Boolean.TRUE.equals(response.get("doNotDisassemble")),
                userId);
    ...
    private void requireDeviceAccess(UUID deviceId, UUID orgId, UUID userId) {
        Integer c = jdbc.queryForObject(
                ...
                "where d.id = ? and d.organization_id = ? and m.user_id = ?",
                Integer.class, deviceId, orgId, userId);
        if (c == null || c == 0) throw new AccessDeniedException("Device access denied");
    }
