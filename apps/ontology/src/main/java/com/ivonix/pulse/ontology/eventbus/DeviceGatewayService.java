...
public class DeviceGatewayService {
    ...
    public DeviceGatewayService(JdbcTemplate jdbc, ObjectMapper mapper) {
        this.jdbc = jdbc;
        this.mapper = mapper;
    }
    ...
    public IngestResult ingest(DeviceMessage message, String credential, String headerDeviceId) {
        ...
        if (!Set.of("TELEMETRY", "HEARTBEAT", "STATE", "ALARM").contains(message.messageType()))
            throw new IllegalArgumentException("Unsupported device message type");

        DeviceIdentity device = authenticate(credential);
        String expected = device.deviceId().toString();
        if (!expected.equals(message.deviceId()) || (headerDeviceId != null && !expected.equals(headerDeviceId)))
            throw new SecurityException("Device identity mismatch");

        // Atomically reject replays and out-of-order messages. NULL means no message has been accepted yet.
        int advanced = jdbc.update("update public.key_devices set last_sequence=?,last_seen_at=now() where id=? and status='active' and (last_sequence is null or last_sequence < ?)",
                message.sequence(), device.deviceId(), message.sequence());
        if (advanced != 1) {
        ...
        payload.put("sentAt", message.sentAt());
        payload.put("sequence", message.sequence());
        payload.put("payload", message.payload() == null ? Map.of() : message.payload());
        payload.put("protocol", message.protocol());
        payload.put("keyId", message.keyId());

        try {
            jdbc.update("insert into public.events(id,event_type_id,organization_id,source_service,event_time,payload,correlation_id,version,external_event_id) values (?,?,?,?,?::timestamptz,?::jsonb,?,?,?)",
                    eventId, eventType, device.organizationId(), "device-gateway", message.sentAt() == null ? Instant.now() : Instant.parse(message.sentAt()),
                    mapper.writeValueAsString(payload), message.messageId(), 1, message.messageId());
    ...
    private DeviceIdentity authenticate(String credential) {
        String hash = sha256(credential.trim());
        try {
            return jdbc.queryForObject("select id,organization_id from public.key_devices where credential_hash=? and status='active'",
                    (rs,n) -> new DeviceIdentity((UUID) rs.getObject("id"), (UUID) rs.getObject("organization_id")), hash);
        } catch (org.springframework.dao.EmptyResultDataAccessException e) {
            throw new SecurityException("Invalid device credential");
        }
    }

    private UUID findExisting(UUID organizationId, String messageId) {
        try {
            return jdbc.queryForObject("select id from public.events where organization_id=? and external_event_id=?", UUID.class, organizationId, messageId);
        } catch (org.springframework.dao.EmptyResultDataAccessException e) {
            return null;
        }
    }
    ...
    private String sha256(String value) {
        try {
            ...
            throw new IllegalStateException("Credential hashing unavailable", e);
        }
    }

    public record DeviceMessage(String schemaVersion, String messageId, String deviceId, String messageType,
                                String sentAt, long sequence, Map<String,Object> payload, String protocol, String keyId) {}
