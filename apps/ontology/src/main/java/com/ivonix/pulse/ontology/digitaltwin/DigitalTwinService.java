...
public class DigitalTwinService {
    private final JdbcTemplate jdbc; private final RestTemplate rest; private final ObjectMapper mapper; private final String simulationUrl;
    public DigitalTwinService(JdbcTemplate jdbc, RestTemplate rest, ObjectMapper mapper, org.springframework.core.env.Environment env) {
        this.jdbc=jdbc; this.rest=rest; this.mapper=mapper; this.simulationUrl=env.getProperty("PULSE_SIMULATION_URL","http://127.0.0.1:8091/simulate");
    }
    @Transactional
    ...
    public UUID runSimulation(UUID orgId, UUID userId, UUID twinId, Map<String,Object> input) {
        ...
        try { HttpHeaders h=new HttpHeaders(); h.setContentType(MediaType.APPLICATION_JSON); rest.postForEntity(simulationUrl,new HttpEntity<>(new SimulationRequest(twinId,runId,safeInput),h),String.class); return runId; }
        catch(RuntimeException ex){ jdbc.update("update public.simulation_runs set status='failed',completed_at=now(),output_data=?::jsonb where id=?",json(Map.of("error","simulation worker unavailable")),runId); throw ex; }
    }
    ...
    @Transactional(readOnly=true)
    public List<Map<String,Object>> predictions(UUID orgId,UUID userId,UUID deviceId){ requireMember(orgId,userId);requireDevice(orgId,deviceId);return jdbc.queryForList("select id,twin_id,failure_type,probability,time_horizon,recommended_action,created_at from public.predicted_failures where device_id=? order by created_at desc limit 50",deviceId); }
    @Transactional(readOnly=true)
