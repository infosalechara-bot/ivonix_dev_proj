...
public class KeyService {
    ...
    public KeyView create(UUID userId, UUID orgId, KeyRequest r) {
        ...
        if (!Set.of("encryption", "signing", "key_exchange").contains(purpose)) {
        ...
        jdbc.update(
            "insert into public.crypto_keys(id, organization_id, key_alias, key_type, purpose, hsm_backed, public_key, encrypted_private_key, created_by) " +
            "values (?,?,?,?,?,?,?,?,?)",
    ...
    public String encrypt(UUID userId, UUID orgId, UUID keyId, String plaintext) {
        ...
        if (r == null || r.ciphertext() == null) {
            throw new IllegalStateException("Crypto service returned no ciphertext");
        }
    ...
    public String decrypt(UUID userId, UUID orgId, UUID keyId, String ciphertext) {
        ...
        if (r == null || r.plaintext() == null) {
            throw new IllegalStateException("Crypto service returned no plaintext");
        }
    ...
    public KeyView get(UUID id, UUID userId, UUID orgId) {
        ...
        return jdbc.queryForObject(
            ...
            "from public.crypto_keys where id=? and organization_id=?",
            (rs, n) -> view(rs), id, orgId
        );
    }
    ...
    private KeyView authorizeKey(UUID id, UUID userId, UUID orgId, String purpose) {
        ...
        if (purpose != null && !purpose.equals(k.purpose())) {
            throw new SecurityException("Key purpose mismatch");
        }
        return k;
    }
    ...
    private KeyView view(java.sql.ResultSet rs) throws java.sql.SQLException {
        return new KeyView(
            ...
            rs.getBoolean("hsm_backed"),
            rs.getString("public_key"),
            rs.getObject("created_at", java.time.Instant.class),
            rs.getObject("last_rotated_at", java.time.Instant.class),
    ...
    private void audit(UUID id, String op, UUID user, String status) {
        jdbc.update(
            "insert into public.crypto_operations(key_id,operation_type,requested_by,status) values (?,?,?,?)",
            id, op, user, status
        );
    }
    ...
    private KeyMaterial generate(String type) {
        try {
            if ("aes-256".equals(type)) {
                byte[] b = new byte[32];
                random.nextBytes(b);
                return new KeyMaterial(null, b);
            }
            KeyPairGenerator g = KeyPairGenerator.getInstance("RSA");
            g.initialize(2048);
            KeyPair p = g.generateKeyPair();
            return new KeyMaterial(
                Base64.getEncoder().encodeToString(p.getPublic().getEncoded()),
                p.getPrivate().getEncoded()
            );
        } catch (Exception e) {
    ...
    private String seal(byte[] plain) {
        try {
            byte[] iv = new byte[12];
            random.nextBytes(iv);
            Cipher c = Cipher.getInstance("AES/GCM/NoPadding");
            c.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(masterKey, "AES"), new GCMParameterSpec(128, iv));
            ...
            throw new IllegalStateException("Private material encryption failed", e);
        }
    }
