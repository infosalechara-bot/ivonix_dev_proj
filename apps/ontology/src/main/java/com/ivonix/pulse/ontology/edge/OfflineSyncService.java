...
@Service public class OfflineSyncService {
 ...
 @Scheduled(fixedDelayString="${PULSE_EDGE_SYNC_DELAY_MS:60000}") public void syncPending(){if(!online())return;try{for(var row:local.pending(50)){long id=(Long)row.get("id");try{cloud.update("select public.apply_sync_record(?::text,?::uuid,?::uuid,?::text,?::jsonb)",row.get("table_name"),UUID.fromString((String)row.get("record_id")),UUID.fromString((String)row.get("organization_id")),row.get("operation"),row.get("data"));local.markSynced(id);}catch(Exception e){local.markFailed(id,e.getMessage());}}}catch(Exception ignored){}}
