...
public class AppForgeService {
     ...
     * database transaction. A provisioning failure therefore cannot publish
     * an app definition with only a subset of its generated tables.
     */
    @Transactional
    public UUID createApp(UUID org,UUID user,String name,String description,List<ModelDef> models){
        ...
        for(ModelDef m:models)validateModel(m);
