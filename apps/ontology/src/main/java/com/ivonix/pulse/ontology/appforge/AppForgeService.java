...
public class AppForgeService {
    ...
    public UUID createApp(UUID org,UUID user,String name,String description,List<ModelDef> models){
        ...
        for(ModelDef m:models){
            ...
            db.execute("create policy "+q("appforge_"+table+"_tenant")+" on "+q(table)+" for all to authenticated using (public.is_org_member((select auth.uid()), organization_id)) with check (public.is_org_member((select auth.uid()), organization_id))");
            db.execute("create index if not exists "+q(table+"_org_created_idx")+" on "+q(table)+" (organization_id, created_at desc)");
            try{db.update("insert into public.app_generated_tables(app_id,table_name,schema) values(?,?,?::jsonb)",app,table,json.writeValueAsString(m));}catch(Exception e){throw new IllegalStateException("Could not register generated table",e);}
        }
        return app;
    }
    ...
    private void member(UUID o,UUID u){if(!Boolean.TRUE.equals(db.queryForObject("select public.is_org_member(?,?)",Boolean.class,u,o)))throw new SecurityException("Organization access denied");}
