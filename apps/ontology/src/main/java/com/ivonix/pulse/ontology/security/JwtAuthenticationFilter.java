...
@Component public class JwtAuthenticationFilter extends OncePerRequestFilter {
 ...
 @Override protected void doFilterInternal(HttpServletRequest req,HttpServletResponse res,FilterChain chain)throws ServletException,IOException{String h=req.getHeader("Authorization");if(h!=null&&h.startsWith("Bearer "))try{Claims c=jwt.claims(h.substring(7));var auth=new UsernamePasswordAuthenticationToken(jwt.userId(c),null,List.of(new SimpleGrantedAuthority("ROLE_USER")));auth.setDetails(jwt.organizationId(c));SecurityContextHolder.getContext().setAuthentication(auth);}catch(Exception ignored){SecurityContextHolder.clearContext();}chain.doFilter(req,res);}
}
