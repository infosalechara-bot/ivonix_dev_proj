...
public class UniversalKeyService {
    ...
    private static final int DEVICE_TOKEN_DAYS = 30;
    ...
    public ActivationCode createActivationCode(UUID userId, UUID orgId, UUID keyId, int ttlMinutes, int maxUses, String[] scopes) {
        ...
        String code = randomCode(); Instant expires = Instant.now().plus(ttlMinutes, ChronoUnit.MINUTES);
        jdbc.update("insert into public.key_activation_codes(organization_id,key_id,code_hash,code_prefix,scope,max_uses,expires_at,created_by) values (?,?,?,?,?,?,?,?)", orgId, keyId, sha256(code), code.substring(0, 4), normalized, maxUses, expires, userId);
    ...
    public ActivationResult activate(String code, String deviceName, String deviceType, String publicKey) {
        ...
        UUID activationId = (UUID) row.get("id"); Instant expires = toInstant(row.get("expires_at"));
        int used = ((Number) row.get("used_count")).intValue(), max = ((Number) row.get("max_uses")).intValue();
        if (row.get("revoked_at") != null || !expires.isAfter(Instant.now()) || used >= max) throw new SecurityException("Activation code unavailable");
        UUID orgId = (UUID) row.get("organization_id"), keyId = (UUID) row.get("key_id"); String[] scopes = array((java.sql.Array) row.get("scope"));
    ...
    public ClientCredential createClient(UUID userId, UUID orgId, String name, String clientType, String[] scopes) {
        requireMember(orgId, userId); String cid = "pk_" + randomToken().substring(0, 20), secret = randomToken(); String[] normalized = normalizeScopes(scopes); UUID id = UUID.randomUUID();
