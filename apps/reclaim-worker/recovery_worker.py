...
app = FastAPI(title="PULSE RECLAIM Worker", version="1.1")

SUPABASE_URL = os.environ["SUPABASE_URL"].rstrip("/")
SERVICE_KEY = os.environ["SUPABASE_SERVICE_ROLE_KEY"]
WORKER_SECRET = os.environ.get("PULSE_RECLAIM_WORKER_SECRET", "")
if not WORKER_SECRET:
    raise RuntimeError("PULSE_RECLAIM_WORKER_SECRET is required")
...
def collect_outputs(output: Path):
    ...
    for p in output.rglob("*"):
        if p.is_file():
            rel=p.relative_to(output); items.append({"item_type":"file","original_location":str(rel),"recovered_data":str(p),"confidence":0.70,"sha256":hash_file(p),"size":p.stat().st_size})
    return items
...
def insert_item(job_id,item):
    api("POST","recovered_items",json={"recovery_job_id":job_id,"item_type":item["item_type"],"original_location":item.get("original_location"),"recovered_data":item.get("recovered_data"),"confidence":item.get("confidence")})
def run_recovery(req: RecoveryRequest):
    output=OUTPUT_ROOT/req.jobId
    try:
        update(req.jobId,"scanning",10); log(req.jobId,"scan_started",{"job_type":req.jobType})
...
async def recover(req: RecoveryRequest, background_tasks: BackgroundTasks, x_pulse_reclaim_secret: Optional[str] = Header(default=None)):
    if x_pulse_reclaim_secret is None or not hmac.compare_digest(x_pulse_reclaim_secret, WORKER_SECRET):
