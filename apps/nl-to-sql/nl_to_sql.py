...
app=FastAPI(title="PULSE Insight NL-to-SQL", version="1.0.0")
...
def convert(req:NLRequest):
    if not UUID_RE.fullmatch(req.organization_id): raise HTTPException(status_code=400,detail="Invalid organization id")
    sql=build(req.text,req.organization_id); log_history(req.text,sql,req.organization_id); return {"sql":sql,"internal":True}
